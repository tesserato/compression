...
/// <param name="W"></param>
/// <returns></returns>
mode_abdm average_pc_waveform(v_real& waveform, v_inte& Xp, const v_real& W) {
#ifdef v
	...
	for (pint i = 0; i < waveform.size() - 1; i++) {
		distance = std::abs(waveform[i]) + std::abs(waveform[i + 1]);
		if (distance < min_distance) {
			min_distance = distance;
			first = i + 1;
		}
	...
	std::rotate(waveform.begin(), waveform.begin() + first, waveform.end());
	//#pragma loop(hint_parallel(4))
	for (pint i = 0; i < Xp.size(); i++) {
		Xp[i] = Xp[i] + first;
	}
...
/// </summary>
/// <param name="W"></param>
/// <param name="avgpc"></param>
/// <param name="min_size"></param>
/// <param name="max_size"></param>
/// <returns></returns>
v_inte refine_Xpcs(const v_real& W, const v_real& waveform, pint min_size, pint max_size) {
	#ifdef v	
	...
	pint best_size{ 0 };
	inte best_x0{ 1 - (inte)min_size };
	real best_val{ 999999999999.0 };
	real curr_val{ 0.0 };
	real step{ 0.0 };
	real amp{0.0};
	v_inte Xpc;
	v_real errors;
	std::vector<v_real> interps(max_size - min_size + 1, v_real(max_size)); //TODO
	for (pint size = min_size; size <= max_size; size++) {
		step = 1.0 / real(size);
		// calculating interpolated waveform and storing into pre calc jagged matrix
		for (pint i = 0; i <= size; i++) { 
			interps[size - min_size][i] = spline(i * step);
		}

		for (inte x0 = 10 - min_size; x0 <= 0; x0++) {
			amp = std::abs(*std::max_element(W.begin(), W.begin() + 1 + size - min_size, abs_compare));
			curr_val = 0.0;
			for (pint i = 0; i <= size + x0; i++) {
				curr_val += pow(W[i] / amp - interps[size - min_size][ - x0 + i], 2.0);
			}
			curr_val /= (real)size;
	...
	Xpc.push_back(best_x0);
	inte curr_x0{ best_x0 + inte(best_size) };
	Xpc.push_back(curr_x0);

	//std::cout << best_x0 << " " << curr_x0 << "\n";

	#ifdef v
	errors.push_back(best_val);
	std::cout << "refine_Xpcs: before while loop\n";
	#endif
	while (curr_x0 + max_size < W.size()) {
		best_val = 999999999999.0; //TODO
		for (pint size = min_size; size <= max_size; size++) {
			amp = std::abs(*std::max_element(W.begin() + curr_x0, W.begin() + curr_x0 + size, abs_compare));
			curr_val = 0.0;
			for (pint i = 0; i <= size; i++) {
				curr_val += pow(W[curr_x0 + i] / amp - interps[size - min_size][i], 2.0);
			}
			curr_val /= (real)size;
			if (curr_val < best_val) {
				best_val = curr_val;
	...
	#ifdef v
	assert(Xpc[1] >= 0);
	write_vector(errors, "errors.csv");
	#endif
	return Xpc;
}
...
std::tuple<v_real, v_real> reconstruct(const v_inte& Xp, const v_real& waveform, const v_real& W) {
	#ifdef v
		std::cout << "inside Compressed::raw\n";
	#endif

	boost::math::interpolators::cardinal_cubic_b_spline<real> spline(waveform.begin(), waveform.end(), 0.0, 1.0 / (real)waveform.size());
	v_real W_reconstructed(W.size(), 0.0);
	v_real Envelope;
	...
	for (pint i = 1; i < Xp.size() - 1; i++) {
		...
		x1 = Xp[i + 1];
		step = 1.0 / real(x1 - x0);
		e = std::abs(*std::max_element(W.begin() + x0, W.begin() + x1, abs_compare));
		Envelope.push_back(e);
		for (inte j = x0; j < x1; j++) {
			W_reconstructed[j] = spline((j - x0) * step) * e;
		}
	...
	Envelope.push_back(std::abs(*std::max_element(W.begin() + Xp.back(), W.end(), abs_compare)));
	#ifdef v
		std::cout << "inside Compressed::raw-1\n";
	#endif
	v_inte Xp_new = Xp;
	...
	for (inte j = x0; j < x1; j++) {
		...
		std::cout << "inside Compressed::raw-5\n";
	#endif
	return { W_reconstructed, Envelope };
}
